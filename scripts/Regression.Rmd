...
start.time <- Sys.time()
for(num in c(50:64)){
  file.create(glue("../claires-secret-folder/sig_vals/std2/{num}.txt"))
  files <- list.files(
  ...
  for (fl in files){
    ...
    tryCatch({
      ...
      species = print_h5py_file(
                glue("../trees/brain2/{num}/species_{i}.h5"), 
                "species")
      species_internal = print_h5py_file(
                glue("../trees/brain2/{num}/species_internal_{i}.h5"), 
                "species_internal", 
      ...
      write(glue("{i}, {delta}, {tmp[3]}, {tmp[4]}"), 
            file = glue("../claires-secret-folder/sig_vals/std2/{num}.txt"), 
            append=TRUE)
